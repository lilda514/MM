...
class Info(API):
    def __init__(self, base_url=None, skip_ws=False):
        ...
        print(f'Info() being called from thread: {threading.current_thread().name}')
    ...
    def close(self):
        if not self.skip_ws:
            ...
            print(f"The ws thread is alive: {self.ws_manager.is_alive()}")
            print(f"The ping thread is alive: {self.ws_manager.ping_sender.is_alive()}")
        self.session.close()
